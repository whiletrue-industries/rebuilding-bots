...
class OpenAIVectorStore(VectorStore):
    def __init__(self, production: bool = False):
        ...
        self._filename_mapping = {}  # Add this to store filename mappings
    ...
    async def _upload_single_file(self, file_tuple: Tuple[str, bytes], session: aiohttp.ClientSession, retries=3):
        ...
        for attempt in range(retries):
            try:
                # Create a new file object for each attempt
                ...
                # Create form data with explicit UTF-8 encoding
        ...
        try:
            ...
            for doc in documents:
                if isinstance(doc, tuple):
                    ...
                    # Handle file content with explicit UTF-8 encoding
            ...
            for i in range(0, total_files, self.batch_size):
                batch = document_data[i:i + self.batch_size]
                logger.info(f'Processing batch {i//self.batch_size + 1} of {(total_files + self.batch_size - 1)//self.batch_size} '
    ...
    def list(self) -> List[dict]:
        """List all vector stores with decoded filenames"""
        try:
            vector_stores = self.client.beta.vector_stores.list()
            return [
                {
                    'id': vs.id,
                    'name': urllib.parse.unquote(vs.name),  # Decode the name
