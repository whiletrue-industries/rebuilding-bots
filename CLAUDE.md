...
Removed in 2026-05-08 — the `agent_tool_overrides` table created by alembic `0009_unified_prompt_editor` is now inert. Tool descriptions resolve from `specs/<bot>/config.yaml` (`name:` for context-search tools) and `specs/openapi/<tool>.yaml` (`description:` for OpenAPI tools); the only way to change them is to edit those files and republish. The table itself is left in place to keep `0009` reversible until a future migration drops it.

The `agent_prompt_snapshots` view from `0009` is still in active use for prompt-section rollback in the unified prompt editor.
